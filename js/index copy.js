...
note();
function note() {
	// 先判斷localStorage上上否有資料  有json資料就抓取並轉成js
	...
	for (i = 0; i < noteData.length; i++) {
		...
		if (now === 1) {
			...
			noteResult += `<li id="todo${i}" class="noteLi yellow">
			<span>${noteData[i]}</span>
			<button id=`+ reseltBtn + ` onclick="reseltListTodo(` + i + `)">
			<i class="fa-solid fa-check" style="color: #00ff59;"></i></button>
			</li>`;
			...
			noteResult += `<li id="todo${i}" class="noteLi green">
			<span>${noteData[i]}</span>
			<button id=`+ reseltBtn + ` onclick="reseltListTodo(` + i + `)">
			<i class="fa-solid fa-check" style="color: #00ff59;"></i></button>
			</li>`;
	...
	// 輸入進html
	noteListUl.innerHTML = noteResult;
	const todoJson = JSON.stringify(noteData);
	localStorage.setItem("noteData", todoJson);
...
function reseltListTodo(myid) {
	...
	console.log(noteData[myid]);
	noteData.splice(myid, 1);
